...
public class Board extends Table {
	...
	@Query
	public static CompletableFuture<List<Board>> boards(ApiContext context, @Id String organisationId) {
		return context.getDatabase()
			.query(Board.class)
			.thenApply(boards -> boards.stream()
				.filter(board -> organisationId.equals(board.getOrganisationId()))
				.toList());
	}
	...
	public static CompletableFuture<Board> putBoard(ApiContext context, @Id String organisationId, @Id Optional<String> boardId, BoardInput input) {
		...
		if(boardId.isEmpty()) {
			...
			board.setOrganisationId(organisationId);
